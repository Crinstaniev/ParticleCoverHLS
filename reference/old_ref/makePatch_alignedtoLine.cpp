...
public:
  ...
  Environment(double top_layer_limI = 100.0, double beam_axis_limI = 15.0,
              int num_layersI = 5,
              vector<double> radiiI = {5.0, 10.0, 15.0, 20.0, 25.0}) {
...
public:
  ...
  wedgeSuperPoint(vector<Point> pointsI) {
    ...
    // maximum size of z_list is the size of pointsI
...
// DEBUG function for superpoint
ostream &operator<<(ostream &os, const wedgeSuperPoint &sp) {
  os << "superpoint_s: " << endl;
  os << "  min: " << sp.min << endl;
  os << "  max: " << sp.max << endl;
  os << "  z_values: ";
  for (int i = 0; i < sp.z_values.size(); i++) {
    os << sp.z_values[i] << " ";
  }
  os << endl;
  // os << "  points: " << endl;
  // for (int i = 0; i < sp.points.size(); i++) {
  //   os << sp.points[i].z << endl;
  // }
    ...
    for (int i = 0; i < env.num_layers; i++) {
      ...
      if (leftRight == true) {
        ...
        if ((start_index + ppl) > (right_bound + 1)) {
          init_patch.push_back(
              wedgeSuperPoint({row_data[i].begin() + right_bound + 1 - ppl,
                               row_data[i].begin() + right_bound + 1}));
        } else {
          init_patch.push_back(
              wedgeSuperPoint({row_data[i].begin() + start_index,
        ...
        if ((start_index - ppl + 1) < left_bound) {
          init_patch.push_back(
              wedgeSuperPoint({row_data[i].begin() + left_bound,
                               row_data[i].begin() + left_bound + ppl}));
        } else {
          init_patch.push_back(
              wedgeSuperPoint({row_data[i].begin() + start_index + 1 - ppl,
